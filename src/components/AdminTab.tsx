...
export const AdminTab: React.FC<AdminTabProps> = ({
  ...
  const currentBlancoArticles = getBlancoChoiceArticleList(config);

  const handleAddBlancoArticle = (artNrStr: string) => {
    if (!artNrStr.trim()) return;
    const toAdd = artNrStr.split(/[,;\s\n]+/).map(s => s.trim()).filter(Boolean);
    const updated = Array.from(new Set([...currentBlancoArticles, ...toAdd]));
    const updatedStr = updated.join(', ');
    onUpdateConfig('importBlancoChoiceArticles', updatedStr);
    if (updated.length > 0) onUpdateConfig('importBlancoChoiceArt1', updated[0]);
    if (updated.length > 1) onUpdateConfig('importBlancoChoiceArt2', updated[1]);
    setNewBlancoArtInput('');
  };

  const handleRemoveBlancoArticle = (artNr: string) => {
    const updated = currentBlancoArticles.filter(a => a.toLowerCase() !== artNr.toLowerCase());
    const updatedStr = updated.join(', ');
    onUpdateConfig('importBlancoChoiceArticles', updatedStr);
    onUpdateConfig('importBlancoChoiceArt1', updated[0] || '');
    onUpdateConfig('importBlancoChoiceArt2', updated[1] || '');
  };

  const handleDirectUpdateBlancoArticles = (val: string) => {
    onUpdateConfig('importBlancoChoiceArticles', val);
    const parts = val.split(/[,;\s\n]+/).map(s => s.trim()).filter(Boolean);
    if (parts.length > 0) onUpdateConfig('importBlancoChoiceArt1', parts[0]);
    if (parts.length > 1) onUpdateConfig('importBlancoChoiceArt2', parts[1]);
  };
            ...
            >
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  ...
                  {/* Blanco Choice Artikelnummern Verwaltung (Ohne Duplizierung) */}
                  <div className="space-y-2.5 pt-1 border-t border-slate-100 dark:border-zinc-800/80">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5">
                        <label className="text-[9px] font-black text-slate-500 uppercase block">
                          Blanco Choice Artikelnummern (Ausnahmen für 3.000,- € VK)
                        </label>
                        <span className="px-1.5 py-0.5 rounded-full text-[9px] font-bold bg-purple-500/10 text-purple-600 dark:text-purple-400">
                          {currentBlancoArticles.length} aktiv
                        </span>
                      </div>
                      <div className="flex items-center gap-1">
                        <button
                          type="button"
                          onClick={() => setShowBlancoRawInput(!showBlancoRawInput)}
                          className="text-[9px] text-purple-600 dark:text-purple-400 hover:underline px-1.5 py-0.5 rounded hover:bg-purple-50 dark:hover:bg-purple-950/40 font-medium transition-colors"
                        >
                          {showBlancoRawInput ? 'Tags-Ansicht' : 'Als Textzeile bearbeiten'}
                        </button>
                        <div 
                          className="relative"
                          onMouseEnter={() => setActiveTooltip('importBlancoChoice')}
                          onMouseLeave={() => setActiveTooltip(null)}
                        >
                          <button
                            type="button"
                            onClick={(e) => {
                              e.stopPropagation();
                              setActiveTooltip(activeTooltip === 'importBlancoChoice' ? null : 'importBlancoChoice');
                            }}
                            className="p-1 rounded-md transition-colors text-slate-400 hover:text-purple-500 hover:bg-slate-100 dark:hover:bg-zinc-850"
                            title="Hilfe zu Blanco Choice"
                          >
                            <HelpCircle className="w-3.5 h-3.5" />
                          </button>
                          <AnimatePresence>
                            {activeTooltip === 'importBlancoChoice' && (
                              <motion.div
                                ...
                                animate={{ opacity: 1, y: 0, scale: 1 }}
                                exit={{ opacity: 0, y: 8, scale: 0.95 }}
                                className="absolute right-0 top-full mt-1 w-80 p-3.5 bg-white dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 shadow-xl rounded-xl text-[10px] text-slate-600 dark:text-slate-400 space-y-1.5 leading-relaxed z-50 pointer-events-none"
                              >
                                <p className="font-bold text-purple-600 dark:text-purple-400">Blanco Choice Armaturen & Farben:</p>
                                <p>
                                  Blanco Choice Armaturen gibt es in vielen unterschiedlichen Farbvarianten (z.B. Mattschwarz, Edelstahl gebürstet, Satin Dark, Roségold) mit jeweils eigenen Artikelnummern.
                                </p>
                                <p>
                                  Hinterlegen Sie hier <strong>alle gewünschten Artikelnummern</strong>. Sobald eine dieser Nummern im CARAT-Import erkannt wird, bucht die App diese als <strong>Wassersystem (mit 3.000,- € VK)</strong> ein und verhindert, dass der Einkaufsbetrag fälschlicherweise in den Gesamt-Möbel-EK einfließt.
                                </p>
                              </motion.div>
                            )}
                          </AnimatePresence>
                    ...
                    {showBlancoRawInput ? (
                      <div className="space-y-1">
                        <input
                          type="text"
                          value={config.importBlancoChoiceArticles ?? `${config.importBlancoChoiceArt1 || ''}${config.importBlancoChoiceArt2 ? ', ' + config.importBlancoChoiceArt2 : ''}`}
                          onChange={(e) => handleDirectUpdateBlancoArticles(e.target.value)}
                          className="input-field input-field-compact text-xs font-mono text-slate-600 dark:text-slate-300 w-full"
                          placeholder="z.B. 527656, 527660, 527657, 527658"
                          autoFocus
                        />
                        <p className="text-[9px] text-slate-400 dark:text-zinc-500">
                          Geben Sie die Artikelnummern durch Komma getrennt ein.
                        </p>
                      </div>
                    ) : (
                      <>
                        {/* Chips der hinterlegten Artikelnummern */}
                        <div className="flex flex-wrap gap-1.5 p-2 rounded-lg bg-slate-50 dark:bg-zinc-900/60 border border-slate-200/80 dark:border-zinc-800/80 min-h-[38px] items-center">
                          {currentBlancoArticles.length === 0 ? (
                            ...
                            currentBlancoArticles.map((artNr, idx) => (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleRemoveBlancoArticle(artNr)}
                                  className="ml-0.5 p-0.5 rounded text-purple-400 hover:text-red-500 hover:bg-purple-100 dark:hover:bg-purple-900/50 transition-colors"
                                  title={`Artikelnummer ${artNr} entfernen`}
                                >
                        ...
                        {/* Schnelles Hinzufügen (einzeln oder mehrere kommagetrennt) */}
                        <div className="flex gap-1.5">
                          <div className="relative flex-1">
                            <input
                              type="text"
                              value={newBlancoArtInput}
                              onChange={(e) => setNewBlancoArtInput(e.target.value)}
                              onKeyDown={(e) => {
                                if (e.key === 'Enter') {
                                  e.preventDefault();
                                  handleAddBlancoArticle(newBlancoArtInput);
                                }
                              }}
                              placeholder="Nummer(n) eingeben (z.B. 527657, 527658)..."
                              className="input-field input-field-compact text-xs font-mono w-full"
                            />
                          </div>
                          <button
                            type="button"
                            onClick={() => handleAddBlancoArticle(newBlancoArtInput)}
                            disabled={!newBlancoArtInput.trim()}
                            className="px-2.5 py-1 text-xs font-semibold rounded-lg bg-purple-600 hover:bg-purple-700 disabled:opacity-40 disabled:hover:bg-purple-600 text-white transition-colors flex items-center gap-1 shadow-xs shrink-0 cursor-pointer disabled:cursor-not-allowed"
                          >
